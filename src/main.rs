...
fn main() -> Result<(), String> {
  ...
  */
  let matches = clap::Command::new("tunnel_inserter")
    .about(
      "Forwards raw packets to <-> from lightway into \
            a tool such as bitripple tunnel.",
    )
    // ----------------- OUTSIDE-CONNECTED FILE DESCRIPTORS -----------------
    .arg(
      arg!(-o --outside <OUTSIDE_FD> "Socket corresponding to outside")
        .value_parser(value_parser!(i32))
        .required(true),
    )
    .arg(
      arg!(-c --control <CONTROL_FD> "Control pipe file descriptor")
        .value_parser(value_parser!(i32))
        .required(true),
    )
    // ---------------------------- IP ADDRESSES ----------------------------
    .arg(
      arg!(--"local-addr" <IP> "Local IPv4 address")
        .value_parser(value_parser!(Ipv4Addr))
        .required(true),
    )
    .arg(
      arg!(--"remote-addr" <IP> "Remote IPv4 address")
        .value_parser(value_parser!(Ipv4Addr))
        .required(true),
    )
    // -------------------------------- PORTS ---------------------------------
    .arg(
      arg!(--"local-ports" <PORTS> "Local ports (space separated)")
        .value_parser(value_parser!(u16))
        .num_args(1..)
        .required(false),
    )
    .arg(
      arg!(--"remote-ports" <PORTS> "Remote ports (space separated)")
        .value_parser(value_parser!(u16))
        .num_args(1..)
        .required(false),
    )
  ...
  */
  ...
  let local_ports: Vec<u16> = matches
    ...
    .map(|ports| ports.copied().collect())
    .unwrap_or_default();
  ...
  if local_ports.len() != remote_ports.len() {
    return Err(
      "Need the same number of --local-port as \
            --remote-port"
        .to_string(),
    );
  }
  ...
  */
  println!("Settings:");
  println!("fd_outside {fd_outside_i}, fd_pipe {fd_pipe_i}");
  println!("cmd: {cmd:?}");
  ...
  */
  ...
  for (l, r) in local_ports.into_iter().zip(remote_ports) {
    ...
    for sock in [&lsock, &rsock] {
      // Set maximum receive/send buffer size to 2 megabytes
  ...
  */
  ...
  let args_interp: Vec<String> = cmd[1..]
    .iter()
    .map(|s| {
  ...
  println!("running command: {} args: {args_interp:?}", cmd[0]);
  let mut cmd0 = Command::new(cmd[0].clone());
  let cmd = cmd0.args(args_interp);
  if let Some(file_name) = stderr {
    let stderr_file = File::create(file_name).expect("Can't create stderr output file");
